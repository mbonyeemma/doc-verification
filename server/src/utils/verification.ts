 ...
 */
export async function checkAspectRatio(buffer: Buffer): Promise<boolean> {
  const image = await Jimp.read(buffer);
 ...
 */
export async function computeImageDifference(buf1: Buffer, buf2: Buffer): Promise<number> {
  ...
  for (let y = 0; y < targetHeight; y++) {
    for (let x = 0; x < targetWidth; x++) {
      const pixel1 = Jimp.intToRGBA(img1.getPixelColor(x, y)).r;
      const pixel2 = Jimp.intToRGBA(img2.getPixelColor(x, y)).r;
 ...
 */
export async function bufferToImage(buffer: Buffer): Promise<Image> {
  return loadImage(buffer);
}

/**
 * Result object for verifyImages.  The status field indicates the first
 * failure encountered or 'verified' if all checks pass.
 */
 ...
 */
export async function verifyImages(params: {
  frontBuffer: Buffer;
  backBuffer: Buffer;
  selfieBuffer: Buffer;
  livenessBuffer: Buffer;
  ...
  try {
    ...
    const idResult = await faceapi
      .detectSingleFace(idImage as any)
      .withFaceLandmarks()
      .withFaceDescriptor();
    const selfieResult = await faceapi
      .detectSingleFace(selfieImage as any)
      .withFaceLandmarks()
      .withFaceDescriptor();
